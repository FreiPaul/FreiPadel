import { api, type Invite, type Settings, type SlotGroup, type User } from './api';
...
async function bootstrap() {
	...
	sync.votes = Object.fromEntries(b.votes.map((v) => [voteKey(v.poll_slot_id, v.user_id), v]));
	...
	sync.slotKeys = Object.fromEntries(b.slot_keys.map((k) => [k, true as const]));
...
function applyDelta(d: Delta) {
	...
	switch (d.entity) {
		...
		case 'slots': {
			...
			sync.slotKeys = Object.fromEntries((p.keys ?? []).map((k) => [k, true as const]));
...
export function slotAvailable(s: SyncPollSlot): boolean {
	return `${s.date}|${s.time}|${s.duration_minutes}|${s.location}` in sync.slotKeys;
}
...
export async function castVote(
	...
	else sync.votes[key] = { poll_slot_id: slotId, user_id: me.id, name: me.name, vote: value };
	try {
		await api.post(`/api/polls/${pollId}/vote`, { poll_slot_id: slotId, vote: value });
	} catch (err) {
...
export async function closePoll(pollId: number, winningSlotId: number | null): Promise<void> {
	await api.post(`/api/polls/${pollId}/close`, { winning_slot_id: winningSlotId });
...
export async function deletePoll(pollId: number): Promise<void> {
	...
	applyDelta({ id: 0, entity: 'poll', entity_id: String(pollId), action: 'delete' });
}
